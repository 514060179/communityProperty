...
const addYear = function(_date, _year) {
	const year = _date.getFullYear() + _year; //获取当前年份


	const mon = _date.getMonth(); //获取当前月份


	const da = _date.getDate(); //获取当前日


	const h = _date.getHours(); //获取小时


	const m = _date.getMinutes(); //获取分钟


	const s = _date.getSeconds(); //获取秒


	const newDate = new Date(year, mon, da, h, m, s);
	return newDate;
};
...
const addMonth = function(_date, _month) {
	...
	const mon = _date.getMonth() + _month; //获取当前月份
...
const addDay = function(_date, _day) {
	...
	const da = _date.getDate() + _day; //获取当前日
...
const addHour = function(_date, _hour) {
	...
	const h = _date.getHours() + _hour; //获取小时
...
const addMinutes = function(_date, _minute) {
	...
	const m = _date.getMinutes() + _minute; //获取分钟
...
const addSeconds = function(_date, _second) {
	...
	const s = _date.getSeconds() + _second; //获取秒
...
// 计费结束时间计算（同pc端）
const dateSubOneDay = function(_startTime, _endTime, feeFlag) {
	if (!_endTime || _endTime == '-') {
		return _endTime
	}
	let dateTime = new Date(_endTime.replace(/\-/g, '/'));
	const startTime = new Date(_startTime.replace(/\-/g, '/'));
	//如果开始时间是31日 结束时间是30日 不做处理
	const _startTimeLastDay = startTime.getDate();
	const _endTimeLastDay = dateTime.getDate();
	if (_startTimeLastDay == 31 && _endTimeLastDay == 30) {
		return formatDate(dateTime);
	}

	//2月份特殊处理
	const _endTimeMonth = dateTime.getMonth();
