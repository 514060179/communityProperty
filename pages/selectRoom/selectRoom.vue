		...
		<view class="cu-bar search bg-white">
			<view class="search-form round">
				...
				<input v-model="searchValue" data-name="searchValue" name="searchValue" @focus="onChange"
					@blur="onSearch" :adjust-position="false" :placeholder="$t('please_enter_the_home_number')"
		...
		<view class="cu-list menu margin-top">
			<view class="cu-item" v-for="(room, idx) in rooms" :key="idx" :label="room.roomId">
				<view class="content padding-tb-sm" is-link @click="chooseRoom(room)">
					<view>
						<text class="cuIcon-clothesfill text-blue margin-right-xs"></text> {{room.roomNum}}室
					</view>
	...
	export default {
		...
		methods: {
			...
			loadRoom: function() {
				...
				loadRooms(this, dataObj, this.rooms.length == 0)
					.then(function(res) {
