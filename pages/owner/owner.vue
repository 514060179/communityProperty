<template>
	<view>
		<view class="q-query flex justify-start flex-wrap">
			<view class="q-item">
				<input type="text" :placeholder="$t('input_building_unit_house')" v-model="roomName"
					confirm-type="search"></input>
			</view>
			<view class="q-item">
				<input type="text" :placeholder="$t('input_owner_name')" v-model="ownerName"
					confirm-type="search"></input>
			</view>
			<view class="q-item">
				<input type="text" :placeholder="$t('input_phone_number')" v-model="link" confirm-type="search"></input>
			</view>
		...
		<view class="cu-modal" :class="delOwnerMemberFlag==true?'show':''">
			<view class="cu-dialog">
				<view class="cu-bar bg-white justify-end">
					<!-- 温馨提示 -->
					<view class="content">{{$t('prompt')}}</view>
				...
				<view class="cu-bar bg-white justify-end">
					...
					<view class="action margin-0 flex-sub text-blue solid-left" @tap="_doDeleteOwnerMember()">
						{{$t('dialog_ok')}}</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			_doDeleteOwnerMember: function() {
				...
				deleteOwner(this, this.curOwnerMember, true)
					.then((_data) => {
						uni.showToast({
							icon: 'none',
							title: _data.msg
