 ...
 * @LastEditTime: 2024-05-23 17:31:01
 * @FilePath: dataReport.vue
		...
		<view class="cu-bar bg-white  solid-bottom" style="justify-content: space-around;">
			<view class="action" @click="changeListType(0)">
				...
				<text class="text-blue"
					:class="{'cuIcon-roundcheckfill' : currentPage == 0, 'cuIcon-roundcheck' : currentPage != 0}"></text>{{ $t('today') }}
			</view>
			<!-- 昨日 -->
			<view class="action" @click="changeListType(1)">
				<text class="text-blue"
					:class="{'cuIcon-roundcheckfill' : currentPage == 1, 'cuIcon-roundcheck' : currentPage != 1}"></text>{{ $t('yesterday') }}
			</view>
			<!-- 近30日 -->
			<view class="action" @click="changeListType(2)">
				<text class="text-blue"
					:class="{'cuIcon-roundcheckfill' : currentPage == 2, 'cuIcon-roundcheck' : currentPage != 2}"></text>{{ $t('last_30_days') }}
			</view>
		</view>
