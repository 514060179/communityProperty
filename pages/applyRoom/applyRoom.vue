	...
	<view>
		<view class="cu-bar bg-white search ">
			<view class="search-form round">
				...
				<input type="text" :placeholder="getHouseNumber" v-model="roomName" confirm-type="search"></input>
			</view>
			<view class="search-form round">
				<text class="cuIcon-search"></text>
				<picker :value="applyStatesIndex" :range="applyStates" :placeholder="getPleace" :range-key="'name'"
		...
		<view class="margin-top" v-if="applyRoomList.length > 0">
			<view class="cu-list menu-avatar " v-for="(item,index) in applyRoomList" :key="index"
				...
				<view class="cu-item arrow">
					<view class="item-content">
						...
						<view class="text-gray text-sm flex">
							<view class="text-cut">
								{{$t('applicant')}}：{{item.createUserName}}-{{item.createUserTel}}
							</view>
						</view>
					</view>
					<view class="action">
...
<script>
	...
	export default {
		...
		computed: {
			getHouseNumber() {
				return this.$t('enter_house_number')
			},
			getPleace() {
		...
		methods: {
			...
			loadApply: function() {
				...
				loadApplyRooms(this, _objData, this.applyRoomList.length == 0)
					.then(function(res) {
			...
			_toApplyRoomDetail: function(_item) {
				uni.navigateTo({
					url: '/pages/applyRoomDetail/applyRoomDetail?ardId=' + _item.ardId + '&communityId=' +
						_item.communityId
				});
			}
		}
