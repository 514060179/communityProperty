			...
			<view class="bg-white margin-bottom padding-sm margin-sm radius-sm" v-for="(item,index) in applyList" :key="index" >
				<view class="apply-title flex justify-between">
					...
					<view class="flex justify-start">
						<button class="cu-btn round sm line-red" v-if="item.state == 1000 && userId == item.createUserId"
	...
	export default {
		...
		methods: {
			...
			_toAddPurchaseApplyPage: function() {
				uni.navigateTo({
					url: '/pages/resource/addPurchaseApply'
				});
			},
			 ...
			 * 紧急采购
			 */
			_toAddUrgentPurchaseApplyPage: function() {
				uni.navigateTo({
					url: '/pages/urgentPurchaseApplyStep/urgentPurchaseApplyStep'
				});
			},
