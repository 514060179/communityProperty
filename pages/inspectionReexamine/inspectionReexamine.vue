<template>
	<view>
		...
		<view class="cu-form-group arrow">
			<view class="title" style="width: 200upx;">{{$t('补检日期')}}</view>
			<uni-datetime-picker type="date" :clear-icon="false" v-model="planInsTime" @change="maskClick"
				:border="false" />
		</view>
		<view v-if="tasks && tasks.length>0" class="margin-top">
			<view v-for="(item,index) in tasks" :key="index"
				...
				<view class="solid-top flex justify-end margin-top padding-top-sm padding-bottom-sm">
					...
					<button class="cu-btn sm bg-blue margin-left" @click="_startInspection(item)">{{$t('补检')}}</button>
				</view>
			</view>
		</view>
		<view v-else>
