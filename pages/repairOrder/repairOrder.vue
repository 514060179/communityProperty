	...
	<view>
		<view class="cu-bar bg-white search ">
			<view class="search-form round">
				...
				<input type="text" :placeholder="$t('input_repairer')" v-model="repairName"
				...
				<view class="cu-item arrow">
					<view class="content content-left">
						<view class="text-grey">
							...
							<text
								class="ellip">{{item.repairObjName}}-{{item.stateName}}({{item.repairSettingTypeName}})</text>
						</view>
						<view class="text-gray text-sm flex">
							<view class="text-cut">
								<!-- 报修人 -->
								{{$t('repairer')}}：{{item.repairName}}-{{item.tel}}
							</view>
						</view>
					</view>
					<!-- 派单 -->
					<view v-if="item.state == 1000 && checkAuth('502019101946430010')"
						class="rob-order text-df text-white bg-blue text-center"
						@tap.stop="dealRepair(item,'DISPATCH')">
						{{$t('dispatch')}}
					</view>
					<view v-if="item.repairWay == 100 && item.state == 1000 && checkAuth('502021012099350016')"
						class="rob-order text-df text-white bg-blue text-center" @tap.stop="_robOrder(item)">
						抢单
					</view>
		...
		methods: {
			...
			_loadRepairOrders: function() {
				...
				this.java110Context.request({
					...
					fail: function(e) {
						wx.showToast({
							title: that.$t('server_error'), //"服务器异常了",
							icon: 'none',
							duration: 2000
						});
					}
				}, this.repairOrders.length == 0);
			},
			...
			_toRepairDetail: function(_item) {
				uni.navigateTo({
					url: "/pages/repairDetail/repairDetail?repairId=" + _item.repairId + '&storeId=' + this
						.storeId
				});
			},
