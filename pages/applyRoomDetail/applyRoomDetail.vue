        ...
        <view class="checkbox-area">
          <checkbox-group @change="checkboxChange">
            <view class="checkbox text-df text-grey" v-for="(item,index) in fees" :key="index">
              <checkbox :value="item.detailId" :checked="item.checked" />【{{item.feeName}}】<text class="text-red">{{item.receivedAmount}}MOP</text> - {{item.createTime}}
            </view>
          </checkbox-group>
        </view>
      </view>
...
export default {
  ...
  onLoad: function (options) {
    ...
    loadApplyRooms(this, _objData, this.sendImgList.length == 0)
      .then(function (res) {
  ...
  methods: {
    ...
    _listFeeDetail: function () {
      ...
      listFeeDetail(this, params, this.fees.length == 0).then(function (res) {
    ...
    discountTypeRangeChange: function (e) {
      ...
      loadFeeDiscount(this, params, this.discountIdRange.length == 0).then(function (res) {
    ...
    submit: function () {
      ...
      if (this.applyRoomInfo.state == 1) {
        ...
        uploadCheckUpdate(this, params, true).then(function (res) {
          uni.hideLoading();
          uni.showToast({
            title: "保存成功"
          })
        ...
        uploadReviewUpdate(this, params, true).then(function (res) {
          uni.hideLoading();
          uni.showToast({
            title: "保存成功"
          })
