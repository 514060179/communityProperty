<template>
	<view>
		<view class="q-query flex justify-start flex-wrap">
			<view class="q-item">
				<input type="text" class="q-input" :placeholder="$t('enter_license_plate_number')"
					v-model="carNumLike"></input>
			</view>
			<view class="q-item">
				<input type="text" class="q-input" :placeholder="$t('enter_owner_name')" v-model="ownerName"></input>
			</view>
			<view class="q-item">
				<input type="text" class="q-input" :placeholder="$t('enter_member_license_plate_number')"
					v-model="memberCarNumLike"></input>
			</view>
			<view class="q-item">
				<input type="text" class="q-input" :placeholder="$t('enter_parking_space_number')"
					v-model="num"></input>
			</view>
			<view class="q-item">
				<input type="text" class="q-input" :placeholder="$t('enter_owner_phone_number')" v-model="link"></input>
			</view>
			...
			<view class="bg-white margin-bottom padding-sm margin-sm radius-sm" v-for="(item,index) in cars"
				...
				<view class="apply-title flex justify-between">
					...
					<view class="flex justify-start" @click="_toTel(item.link)">
						...
						<text
							class="text-blue cuIcon-dianhua photo_icon margin-right-xs"></text>{{item.ownerName}}/{{item.link}}
					</view>
				</view>
				<view class="apply-content flex justify-start flex-wrap">
					<view class="item">
						<!-- 车位 -->
						<text>{{$t('parking_space')}}:</text>
						<text v-if="item.areaNum && item.state == '1001'">{{item.areaNum}}-{{item.num}}</text>
						<!-- 车位已释放 -->
						<text v-else>{{$t('parking_space_released')}}</text>
					</view>
					...
					<view class="item">
						<!-- 房屋号 -->
						<text>{{$t('house_number')}}:</text>
						<text>{{item.roomName || '-'}}</text>
					</view>
					<view class="item">
						<!-- 车牌类型 -->
						<text>{{$t('license_plate_type')}}:</text>
						<!-- 临时车 -->
						<text v-if="item.leaseType == 'T'">{{$t('temporary_car')}}</text>
