<template>
	<view class="user-container">
		<view class="cu-list menu" v-if="notices.length > 0" v-for="(notice, idx) in notices" :key="idx"
			:data-item="notice" @click="gotoDetail(notice)">
			<view class="cu-item arrow">
				<view class="content padding-tb-sm">
					...
					<view class="text-gray text-sm">
						<!-- 发布时间 -->
						<text class="margin-right-xs">{{$t('release_time')}}：</text> {{notice.timeStr}}
					</view>
				</view>
			</view>
		</view>
		<noDataPage v-if="notices.length === 0"/>
	</view>
</template>
