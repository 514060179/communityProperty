 ...
 * @LastEditTime: 2024-05-24 09:20:39
 * @FilePath: use-integral-log.vue
    ...
    <view v-if="orders && orders.length>0">
      <view v-for="(item,index) in orders" :key="index" class="bg-white margin-top margin-right-xs radius margin-left-xs padding">
        <view class="flex padding-bottom-xs solid-bottom justify-between">
          <view style="font-size: 14px;">单号<span style="margin-left: 10px;" class="text-gray">{{item.uoId}}</span></view>
        </view>
        ...
        <view class="flex margin-top justify-between">
          ...
          <view class="text-gray">{{item.useQuantity}}个积分/{{item.money}}MOP</view>
        </view>
